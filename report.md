...
清洗与合并后，多日数据在 `Label` 维度覆盖 14 种攻击类型与 Benign。由于部分类别样本极少（如 SQL Injection、Brute Force -XSS/Web 等），若直接按原始比例抽样会导致稀有类在 3k 规模下几乎不可见，因此本文采用“最小样本数 + 比例补齐”的子集策略：每类至少 100 条，其余样本按原始分布比例分配，最终得到 3,000 条多类别样本。该策略兼顾了“保持总体分布趋势”和“保证稀有类可评估”的双重目标，类别统计明细见 `results.md` 与 `data/processed/ids2018_subset_3k_stats.csv`。
...
实验结果（详见 `results.md` 第 2 节表格）显示：PCA 在 10 维时信息保留率已达 0.9564，15 维与 20 维分别达到 0.9874 与 0.9971，说明所选 30 维核心特征存在较强冗余相关性，线性子空间能够在较低维度下保留绝大多数方差结构。LDA 作为监督降维在多类别场景下保持更高的类间分离度（例如 6 维时为 0.4680），体现了标签信息对可分性的强化作用。t-SNE 在本实验中仅用于二维可视化，其“信息保留率”不适用解释方差口径。
...
在统一的训练/测试划分与超参数网格下，本文完成 18 组“降维×分类”组合评测，完整结果见 `results.md` 第 3 节表格。总体上可以观察到三条规律。
...
为回应“需单独统计高发攻击类型检测精度”的要求，本文对所有攻击类型输出 Precision、Recall 与 F1 分数，详见 `results.md` 第 4 节与 `data/processed/attack_metrics.csv`。在多类别设置下，不同攻击类型的样本量差异明显，模型在高频攻击（如 DDoS/DoS 变种）与稀有攻击（如 SQL Injection、Brute Force -XSS/Web）上的表现存在差异，需要结合类别分布与安全代价进行解读。
